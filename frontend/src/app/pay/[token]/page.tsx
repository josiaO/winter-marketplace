...
export default function PaymentLinkPage() {
  ...
  useEffect(() => {
    if (!token) return;
    api.escrow.paymentLinkDetail(String(token))
      .then(res => {
        setLinkData(res);
  ...
  const handleRequestOtp = async () => {
    ...
    setSubmitting(true);
    try {
      await api.escrow.requestLinkOtp(String(token), { channel, destination });
      toast.success(`Verification code sent to your ${channel}`);
      setStep('otp');
    } catch (err: any) {
      toast.error(err.detail || "Failed to send code");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleVerifyOtp = async () => {
    ...
    setSubmitting(true);
    try {
      await api.escrow.verifyLinkOtp(String(token), { channel, destination, otp });
  ...
  const handlePay = async () => {
    setSubmitting(true);
    try {
      const res = await api.escrow.initiateLinkPayment(String(token), {
        buyer_phone: channel === 'sms' ? destination : '',
      });
      if (res.payment_url) {
        window.location.href = res.payment_url;
      }
    } catch (err: any) {
      toast.error(err.detail || "Payment initiation failed");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  if (!linkData) return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-muted/30 p-6 text-center space-y-4">
        ...
        <Card className="overflow-hidden border-none shadow-2xl rounded-[32px] bg-card/80 backdrop-blur-md">
          <CardContent className="p-0">
            ...
            <div className="bg-emerald-600 p-8 text-white space-y-1 relative overflow-hidden">
              ...
              <h2 className="text-4xl font-black tabular-nums">{formatTZS(linkData.transaction.amount)}</h2>
              ...
              <div className="flex gap-4 items-start">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-muted-foreground line-clamp-2 leading-relaxed">
                    {linkData.description || "You are making a secure payment via SmartDalali Escrow Protection."}
                  </p>
                </div>
              </div>
                  ...
                  >
                    <div className="space-y-4">
                      ...
                      <div className="relative group">
                        <Input 
                          placeholder={channel === 'sms' ? "Phone: 07XXXXXXXX" : "Email: name@example.com"}
                  ...
                  >
                    ...
                    <div className="pt-2">
                      ...
                      <p className="text-center text-[10px] text-muted-foreground mt-4 px-6 leading-relaxed">
                        By clicking pay, you agree to SmarDalali&apos;s Escrow Terms. Funds are held until you confirm receipt.
                      </p>
                    </div>
                  </motion.div>
                )}
